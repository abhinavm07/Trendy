       ...
       // const userFollowing = await client.v2.following(`${userId}`);
        ...
        if (userTweets) {
            ...
            for (const twt of twtData) {
              ...
              tweetsColec.push({id: twt['id:'], tweet: twt["text"], sentiment: twtSentiment});
            }
            return res.status(200).json({userData: user, twtData: tweetsColec});
        }
        return res.status(404).send('User account is not public, please search for a user with public tweets.');
    }
